...
const WinningQuestions = () => {
  ...
  const deleteQuestion = async (questionId: string) => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_BACKEND_URL}/api/admin/winningquestion/${questionId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`
          }
      ...
      toast({
        title: "Success",
        description: "Winning question deleted successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to delete winning question. Please try again.",
        variant: "destructive"
      });
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
        ...
        <Button 
          variant="default"
          onClick={() => navigate("/teacher/create-winning-question")}
...
export default WinningQuestions;
